...
// Benchmark Backup
class Benchmark2 extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      userInfo: {
  ...
  componentDidMount() {
    ...
    var bands = this.props.filters.Band.split(",");
    var valuationValues = this.props.valuationValues;
    ...
    bands.map((val, idx) => {
      var numberOfAwards = valuationValues[0][`accessor_${val}`];
      var licenseTerm = valuationValues[1][`accessor_${val}`];
      var mean = valuationValues[2][`accessor_${val}`];
      var median = valuationValues[3][`accessor_${val}`];
      _val.push([val, numberOfAwards, licenseTerm, mean, median]);
    });

    if (bands.length > 1) {
      var numberOfAwards = valuationValues[0][`allBands`];
      var licenseTerm = valuationValues[1][`allBands`];
      var mean = valuationValues[2][`allBands`];
      var median = valuationValues[3][`allBands`];
      _val.push(["All Bands", numberOfAwards, licenseTerm, mean, median]);
    }

    this.state.valuationResult = _val;
  ...
  downloadData() {
    ...
    if (getLang() == "ar") {
      let arHead8 = [
        [
          ...
          getValue("Term", getLang()),
          getValue("PriceByMean", getLang()) + " " + this.props.filters.Header,
    ...
    doc.addImage(this.state.imageData, "jpeg", 20, 45, (111 * 1400) / 834, 111);
